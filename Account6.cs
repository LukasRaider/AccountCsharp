...
{
    public String name;
    public Date dateOfBirth = new Date(int den,int mesic, int rok);                   //
    ...
    public Person(String name, int den, int mesic, int rok) {
        this.name = name;
        dateOfBirth.day = den;
        dateOfBirth.month = mesic;
        dateOfBirth.year = rok;
    }
    public int GetAge() {
        int newAge;
        newAge = DateTime.Now.Year - dateOfBirth.year;
        return newAge;
    }
    public void writeAccounts()
    {
        ...
        {
            Console.WriteLine(this.dateOfBirth.year);
            Console.WriteLine($"{i}. {GetAge} účet: {accounts[i]}");
...
class TestAccount
{
    public static void Mainx(string[] args)
    {
        ...
        Account u3 = new Account(new Person("Jana", 10, 2, 1996), 500);
